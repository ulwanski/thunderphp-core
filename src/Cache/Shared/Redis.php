...
class Redis implements RedisInterface, EventDispatcherInterface {
    ...
    public function __construct($host = 'localhost', $port = 6379, $pass = null, $persistent_id = null, $timeout = 0, $retry_interval = 0){
        ...
        if($persistent_id === null){
            ...
            $result = $this->redis->connect($host, $port, floatval($timeout), null, $retry_interval);
        } else {
        ...
        if($pass !== null && is_string($pass)){
            ...
            # Check if authenticate was successfully.
     ...
     * @return Redis
     */
    final public static function getInstance(ConfigurationLoader $config, CacheInterface $cache) {

        if (self::$instance == false){
            /** @var ConfigModel $redisConfig */
            $redisConfig  = $config->getCoreConfig('redis');
            $host       = $redisConfig->getValue('host', 'localhost');
            $port       = $redisConfig->getValue('port', 6379);
            $auth       = $redisConfig->getValue('auth');
            $session_id = 'core_'.md5($host.$port.$auth);

            self::$instance = new Redis($host, $port, $auth, $session_id, $redisConfig->timeout, $redisConfig->retry);
        }
