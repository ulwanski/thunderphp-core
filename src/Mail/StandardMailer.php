...
use \Core\Mail\Auth\Smtp as SMTP;

class StandardMailer extends AbstractMail {
     ...
     * @param null $password
     */
    public function setSMTP($host, $port = 25, $auth = false, $username = null, $password = null){
        $this->mailer = 'smtp';
    ...
    public function isSendmail()
    {
        ...
        if (!stristr($ini_sendmail_path, 'sendmail')) {
            ...
            $this->sendmailPath = $ini_sendmail_path;
        }
    ...
    public function isQmail()
    {
        ...
        if (!stristr($ini_sendmail_path, 'qmail')) {
            $this->sendmailPath = '/var/qmail/bin/qmail-inject';
        } else {
            $this->sendmailPath = $ini_sendmail_path;
        }
        $this->mailer = 'qmail';
        ...
        if ($this->idnSupported() and
            ...
            if ($this->has8bitChars($domain) and @mb_check_encoding($domain, $this->charSet)) {
                ...
                if (($punycode = defined('INTL_IDNA_VARIANT_UTS46') ?
                    idn_to_ascii($domain, $param, INTL_IDNA_VARIANT_UTS46) :
    ...
    protected function sendmailSend($header, $body)
    {
        if ($this->Sender != '') {
            if ($this->mailer == 'qmail') {
                $sendmail = sprintf('%s -f%s', escapeshellcmd($this->sendmailPath), escapeshellarg($this->Sender));
            } else {
                $sendmail = sprintf('%s -oi -f%s -t', escapeshellcmd($this->sendmailPath), escapeshellarg($this->Sender));
            }
        } else {
            if ($this->mailer == 'qmail') {
                $sendmail = sprintf('%s', escapeshellcmd($this->sendmailPath));
            } else {
                $sendmail = sprintf('%s -oi -t', escapeshellcmd($this->sendmailPath));
            }
        }
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                if (!@$mail = popen($sendmail, 'w')) {
                    throw new MailerException($this->lang('execute') . $this->sendmailPath, self::STOP_CRITICAL);
                }
