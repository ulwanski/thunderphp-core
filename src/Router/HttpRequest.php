...
namespace Core\Router {
    ...
    class HttpRequest {
        ...
        private $requestLang = null;                                                                                    // Wybrany język klienta
        ...
        private $request_id = false;                                                                                    // Jeżeli żądanie posiadało ID, jest ono przepisywane do tej zmiennej
        ...
        public function __construct() {
            ...
            $availableLangList = array(
                'pl_PL',
                'de_DE',
                'en_GB',
            );

            if(isset($_COOKIE[self::COOKIE_LANGUAGE]) && in_array($_COOKIE[self::COOKIE_LANGUAGE], $availableLangList)){
                $this->requestLang = $_COOKIE['language'];
            } else {
                $this->requestLang = $this->getClientLanguage($availableLangList, 'pl_PL');
                setcookie(self::COOKIE_LANGUAGE, $this->requestLang, time()+60*60*24*30);                              // expire in 30 days
            }
        ...
        private function getClientLanguage($availableLangList, $defaultLanguage) {
